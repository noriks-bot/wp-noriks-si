...
function gck_get_bundle_offers( $product_id = null ) : array {
    ...
    foreach ( $rows as $row ) {
        ...
        $saving_text   = "Skupni prihranek " . number_format($saving_amount, 2, '.', '') . "€";
...
function gck_split_attrs_color_size( array $custom_attrs ) : array {
    ...
    foreach ( $custom_attrs as $key => $attr ) {
        ...
        $is_color = ( strpos($hay, 'stran') !== false || strpos($hay, 'boja') !== false || strpos($hay, 'barva') !== false || strpos($hay, 'farb') !== false || strpos($hay, 'culoare') !== false || strpos($hay, 'color') !== false || strpos($hay, 'colour') !== false || strpos($hay, 'colore') !== false || strpos($hay, 'kolor') !== false || strpos($hay, 'szín') !== false || strpos($hay, 'szin') !== false || strpos($hay, 'ρώμ') !== false || strpos($hay, 'цвят') !== false );
 ...
 */
function gck_sl_majice_phrase( int $n, bool $free = false, string $type = 'majica' ) : string {
    if ( $type === 'steznica' ) {
        $m100 = $n % 100; $m10 = $n % 10;
        if ( $m10 === 1 && $m100 !== 11 ) { $adj = 'brezplačno'; $noun = 'opornico'; }
        elseif ( $m10 === 2 && $m100 !== 12 ) { $adj = 'brezplačni'; $noun = 'opornici'; }
        elseif ( in_array( $m10, array( 3, 4 ), true ) && ! in_array( $m100, array( 13, 14 ), true ) ) { $adj = 'brezplačne'; $noun = 'opornice'; }
        else { $adj = 'brezplačnih'; $noun = 'opornic'; }
    ...
    if ( $is1 ) {
        ...
        $adj = 'brezplačni'; $noun = $stem . 'i';
    } elseif ( $is34 ) {
        $adj = 'brezplačne'; $noun = $stem . 'e';
    } else {
        $adj = 'brezplačnih'; $noun = $stem;
    }
...
function gck_render_bundle_selector() {
    ...
    // SOCKS only. Note: the compression MAJICA (orto-kompresijske-majice) must
    // NOT match here — detect via sock-specific words, not 'kompresijsk',
    // so the shirt keeps the default "majica" noun.
    if (
        has_term( array( 'orto-kompresijske-nogavice', 'orto-kompresijske-carape' ), 'product_cat', $product_id )
    ...
    // extended to 3 garment groups (4 majica + 1 bokserica + 1 carapa/nogavica).
    // Gated strictly to this category. This site's save/sync collapses product
    // attributes to a single Barva/Velikost, so the 3 groups' colour/size options
    // are sourced from the product's real grouped attributes when available and
    // fall back to hardcoded arrays fed into the EXACT SHBOX swatch/select markup
    // + CSS (see the render branch below). Detected early and bypasses the
  ...
  margin-right:6px;
    ...
    <?php
    // SHGIFTS: the nogavice size labels ("39-43" …) are wider than shirt/boxer sizes,
    ...
    <div class="gck-benefits-box">
        ...
        <ul class="gck-benefits-list">
            ...
            <li><strong>✔ Vrhunski materiali in natančen kroj</strong></li>
            <li><strong>✔ Udobje brez kompromisov</strong></li>

            <?php if ( has_term( array( 'orto-starter' ), 'product_cat', $product_id ) ) : ?>
                <li style="color: #c00;"><strong>✔ Začetni paket na voljo samo enkrat na osebo</strong></li>
                <li style="color: #c00;"><strong>✔ Omejeno na 1.000 paketov</strong></li>
            <?php endif; ?>
        </ul>
        <?php endif; ?>

        <?php if ( ! $show_countdown && ! $gck_no_attrs && ! $gck_single_size && ! has_term( array( 'orto-kneefix', 'orto-kompresijske-nogavice', 'orto-leak-boxers', 'orto-kompresijske-majice' ), 'product_cat', $product_id ) ) : ?>
    ...
    <div class="gck-top-banner-wrap">
        <?php if ( has_term( array( 'orto-starter' ), 'product_cat', $product_id ) ) : ?>
           ...
           <span>Izberite svoj ekskluzivni<br/> začetni paket</span>
        </div>
    ...
    <?php if ( has_term( array( 'orto-starter' ), 'product_cat', $product_id ) ) : ?>
        <div class="dev-banner" data-total="1000" data-sold="354">
            <div class="dev-banner__text">
                ...
                <span>Omejena količina <b class="sold">0</b> / <b class="total">0</b> (Še <b class="remain">0</b> kosov)</span>
            </div>
    ...
    <div id="bundle-selector" class="bundle-box<?php echo $gck_single_size ? ' is-single-size' : ''; ?><?php echo $gck_no_attrs ? ' is-no-attrs' : ''; ?>" data-split-garments="<?php echo $gck_split_garments ? '1' : '0'; ?>">
        ...
        foreach ( $offers as $offer_id => $data ) :
            ...
            // SHGIFTS: real piece count is 6 (4 majica + 1 bokserica + 1 nogavica),
        ...
        ?>
            <label style="position: relative; <?php if ( ($loop_index == 1 ||  $loop_index == 3) && ! $show_group_titles) : ?> margin-top: 25px;  <?php endif; ?>"
                ...
                <?php if ( ! $show_group_titles ) : ?>
                    <?php if ( $loop_index == 1 ) : ?>
                        <div class="gck-popular-badge">Najbolj priljubljeno 🔥</div>
                    <?php endif; ?>

                    <?php if ( $loop_index == 3 ) : ?>
                        <div class="gck-popular-badge-2">Najboljša cena 🔥</div>
                    <?php endif; ?>
                <?php endif; ?>
    ...
    && !has_term( array( 'starter-paketi' ), 'product_cat', $product_id )
                ...
                <?php if ( $show_price_highlights ) : ?>
                    ...
                    <span class="gck-per-chip">
                        ...
                        <?php endif; ?>
                        <span class="gck-per-new"><?php echo number_format( $gck_per_new, 2 ); ?>€ / kos</span>
                    </span>
                    <?php if ( $discount_pct > 0 ) : ?>
                        <span class="gck-discount-badge">−<?php echo (int) $discount_pct; ?>%</span>
                    <?php endif; ?>
                <?php else : ?>
                    — <span class="bundle-option-title"><?php echo number_format( $gck_per_new, 2 ); ?>€ / kos</span>
                <?php endif; ?>
                <?php endif; ?>
                ...
                <div class="bundle-total-line">
                    <span style="font-weight:normal;">Skupaj:</span>
...
<div class="bundle-total-line">
    ...
    <span class="line-total"><?php echo number_format( (float) $data['total'], 2 ); ?>€</span>
                ...
                <div class="bundle-pairs <?php echo $is_default ? '' : 'hidden'; ?>"
                    ...
                    // attributes when resolvable, grouped by garment word via the same
                    // $attr_groups the SHBOX path uses; otherwise a hardcoded fallback per
                    // group so it never breaks. Form field names carry the garment token
                    // (majica_/bokserica_/carapa_) so cart/label capture + the socks-width
                    // CSS stay stable. Each item gets its own pair index so all 6
                    // selections reach the cart.
                    $gck_sections = array(
                        array(
                            'label'   => 'Izberi 4 majice',
                            'gratis'  => false, 'count' => 4, 'gg' => 0, 'garment' => 'majica',
                            'ckey'    => 'majica_color', 'skey' => 'majica_size',
                            'colors'  => array( 'Črna', 'Bela', 'Siva', 'Bež', 'Temnomodra', 'Rjava', 'Zelena' ),
                            'sizes'   => array( 'S', 'M', 'L', 'XL', '2XL', '3XL', '4XL' ),
                        ),
                        array(
                            'label'   => 'Izberi 1 bokserice gratis',
                            'gratis'  => true, 'count' => 1, 'gg' => 1, 'garment' => 'bokserica',
                            'ckey'    => 'bokserica_color', 'skey' => 'bokserica_size',
                            'colors'  => array( 'Črna', 'Modra', 'Siva', 'Zelena', 'Rdeča' ),
                            'sizes'   => array( 'S', 'M', 'L', 'XL', '2XL', '3XL', '4XL' ),
                        ),
                        array(
                            'label'   => 'Izberi barvo nogavic (5 parov)',
                            'gratis'  => true, 'count' => 1, 'gg' => 2, 'garment' => 'carapa',
                            'ckey'    => 'carapa_color', 'skey' => 'carapa_size',
                            'colors'  => array( 'Črna', 'Bela' ),
                    ...
                    foreach ( (array) $attr_groups as $grp ) {
                        ...
                        elseif ( strpos( $hay, 'carap' ) !== false || strpos( $hay, 'nogavic' ) !== false ) { $gck_real['carapa'] = array( 'colors' => $cv, 'sizes' => $sv ); }
                    }
                    ...
                    if ( $gck_split_garments ) {
                        $gck_passes = array(
                            array( 'group' => 0, 'label' => 'Izberi ' . (int) $pairs . ' ' . gck_sl_majice_phrase( $pairs, false, 'majica' ), 'gratis' => false ),
                            array( 'group' => 1, 'label' => 'Izberi še ' . (int) $pairs . ' ' . gck_sl_majice_phrase( $pairs, true, 'bokserica' ), 'gratis' => true ),
                        );
                    } else {
                    ...
                    <?php for ( $i = 1; $i <= $pairs; $i++ ) : ?>
                        <?php if ( $gck_pass['group'] === null && $gck_show_sections && $gck_paid > 0 && $i === 1 ) : ?>
                            <div class="gck-pair-label">Izberi <?php echo (int) $gck_paid; ?> <?php echo esc_html( gck_sl_majice_phrase( $gck_paid, false, $gck_garment ) ); ?></div>
                        <?php endif; ?>
                        <?php if ( $gck_pass['group'] === null && $gck_show_sections && $gck_free > 0 && $i === ( $gck_paid + 1 ) ) : ?>
                            <div class="gck-pair-label is-gratis">Izberi še <?php echo (int) $gck_free; ?> <?php echo esc_html( gck_sl_majice_phrase( $gck_free, true, $gck_garment ) ); ?></div>
                        <?php endif; ?>
                        <div class="bundle-pair">
                    ...
                    <?php if ( ! $gck_no_attrs ) : ?>
                    <small style="display: block; line-height: 1.3; margin-top: 14px;"><?php esc_html_e( 'Ponujamo 14 dni za vračilo denarja ali brezplačno zamenjavo izdelka – brezskrbno nakupovanje!
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    form.addEventListener('submit', function (e) {
        ...
        if (!valid) {
            e.preventDefault();
            alert('Prosimo, izberite barvo in velikost za vsak komplet.');
        }
    });
 ...
 * total: majica 1-4 (colour+size), bokserica (colour+size), nogavica (colour+size).
 * Each line is labelled by inspecting its field keys so the cart / order clearly
 * shows which garment was picked, e.g. "Majica: Črna - L", "Bokserica: Modra - M",
 * "Nogavica: Črna - 39-43".
 */
function gck_build_shgifts_lines( array $pairs_data ) : array {
    ...
    foreach ( $pairs_data as $attrs ) {
        ...
        foreach ( array_keys( $attrs ) as $k ) {
            ...
            if ( strpos( $kl, 'carap' ) !== false )    { $label = 'Nogavica';  break; }
        }
...
function gck_add_cart_item_data( $cart_item_data, $product_id, $variation_id ) {
    ...
    // SHGIFTS (orto-majica-darila): 6 items (4 majica + 1 bokserica + 1 nogavica),
...
function gck_shgifts_free_gifts_notice() {
    ...
    // Gifts shown: 1x bokserice + 5x par nogavic. Images + crossed-out reference
    ...
    for ( $i = 0; $i < 5; $i++ ) {
        $gifts[] = array( 'label' => '1x par nogavic', 'img' => $socks_img, 'price' => '12,99 €' );
    }
    ?>
    <style>
      .gck-fg{
    ...
    <div class="gck-fg">
        <div class="gck-fg__head">6 BREZPLAČNIH kosov ob vsakem nakupu</div>
