...
<!-- suport section -->
	...
	<?php
	// ===== Klaviyo newsletter =====
	// TODO(SI): replace with the Slovenian footer list ID (Klaviyo -> Lists
	// & Segments -> Settings -> List ID). Left as placeholder so signups are
	// NOT sent to another market's list until the SI list is set.
	$nf_klaviyo_list = 'TPQpPt';
	...
	if ( ! $nf_brand ) {
		$nf_brand = 'NORIKS oblikuje brezčasne kose za močnejšo postavo — daljše, udobnejše in premišljeno izdelane tam, kjer je najpomembneje.';
	}
	$nf_bg      = get_field( 'footer_bg_image', 'option' ); // background photo behind the brand band
	$nf_legal   = get_field( 'footer_legal_text', 'option' );
	if ( ! $nf_legal ) {
		$nf_legal = 'NORIKS BRAND. Vse pravice pridržane. Cene vključujejo DDV. Slike so ilustrativne narave.';
	}
	?>
		...
		<section class="nf-signup">
			<div class="nf-wrap nf-signup-grid">
				<div class="nf-signup-copy">
					<h2 class="nf-signup-title">Pridruži se NORIKS družini</h2>
					<p class="nf-signup-sub">Ekskluzivne ponudbe in napovedi novih izdelkov — izvej prvi.</p>
				</div>
				<div class="nf-signup-form-col">
					<form class="nf-signup-form" data-klaviyo-list="<?php echo esc_attr( $nf_klaviyo_list ); ?>" novalidate>
						...
						<div class="nf-field-row">
							<input type="email" id="nf-email" name="email" class="nf-input" placeholder="Tvoj email naslov" required autocomplete="email">
							<button type="submit" class="nf-btn nf-btn-light">Prijavi se</button>
						</div>
						<p class="nf-signup-note">
							S prijavo se strinjaš s prejemanjem e-mailov. Odjaviš se lahko kadarkoli.
		...
		<section class="nf-main">
			<div class="nf-wrap nf-main-grid">
				...
				<div class="nf-brand-col">
					<p class="nf-brand-tagline">Oblačila za močnejšo postavo, krojena, da se resnično prilegajo.</p>
						...
						<div class="nf-link-col">
							...
							if ( $nf_col2_l ) :
								foreach ( $nf_col2_l as $item ) :
									if ( false !== strpos( (string) $item['link'], '/navodila' ) ) { $nf_has_upute = true; }
									?>
								<a href="<?php echo esc_url( $item['link'] ); ?>"><?php echo esc_html( $item['text'] ); ?></a>
							<?php endforeach; endif; ?>
							<?php if ( ! $nf_has_upute ) : ?>
								<a href="/si/navodila/">Navodila za uporabo</a>
							<?php endif; ?>
						</div>
					<?php endif; ?>
		...
		<div class="nf-wrap nf-meta">
			<div class="nf-meta-left">
				<span class="nf-region">
					<img loading="lazy" decoding="async" src="https://static.devit.software/countries/flags/rectangle/si.svg" alt="" class="nf-flag">
					Slovenija (SI)
				</span>
				<span class="nf-copy">© <?php echo date( 'Y' ); ?> NORIKS. Vse pravice pridržane.</span>
			</div>
	...
	(function () {
		...
		form.addEventListener('submit', function (e) {
			...
			if (!email || email.indexOf('@') === -1) {
				msg.textContent = 'Vnesi veljaven e-mail naslov.'; msg.className = 'nf-signup-msg is-error'; return;
			}
			if (!list || list === 'REPLACE_LIST_ID') {
				msg.textContent = 'Newsletter še ni povezan (manjka Klaviyo List ID).'; msg.className = 'nf-signup-msg is-error'; return;
			}
			msg.textContent = 'Prijava…'; msg.className = 'nf-signup-msg';
			...
			fetch('https://manage.kmail-lists.com/ajax/subscriptions/subscribe', {
				...
				if (d && d.success) {
					msg.textContent = 'Hvala! Preveri e-mail za potrditev.'; msg.className = 'nf-signup-msg is-ok';
					form.reset();
				} else {
					msg.textContent = (d && d.errors && d.errors[0]) ? d.errors[0] : 'Nekaj je šlo narobe, poskusi znova.';
					msg.className = 'nf-signup-msg is-error';
				}
			}).catch(function () {
				msg.textContent = 'Nekaj je šlo narobe, poskusi znova.'; msg.className = 'nf-signup-msg is-error';
			});
		});
	})();
	</script>
